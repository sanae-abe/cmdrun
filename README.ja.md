...
### 🚀 パフォーマンス
- **起動時間が約17倍高速** - Node.jsベースのタスクランナーと比較
- **起動時間6.5ms（平均）** - npm/yarnの115ms以上と比較して大幅に高速
- **メモリフットプリント4.5MB** - Node.jsの200MB以上と比較して極めて軽量

### 🔒 セキュリティ
- **Rustコード内`eval()`ゼロ** - アプリケーション内での動的コード生成なし
- **安全な変数展開** - シェルインジェクション脆弱性なし
- **細粒度制御** - コマンド連結とサブシェルの階層的権限システム
  - `allow_command_chaining` - `&&`, `||`, `;` 演算子の制御（デフォルト: 無効）
  - `allow_subshells` - 正規表現パターン用の `()` 括弧の制御（デフォルト: 無効）
- **エスケープシーケンス** - `\n`, `\r`, `\t` のサポート（セキュリティリスク最小）
...
**独自の機能の組み合わせ:**
- 🔒 Rust eval()ゼロとfuzzing（373,423テスト、0クラッシュ）
- 🌍 4言語サポート（英・日・簡体中・繁体中）
- 🎨 シェル補完（Zsh/Bash/Fish）
- 📊 SQLiteベース実行履歴
- 🔌 動的プラグインシステム
...
cmdrunは**高速・安全・クロスプラットフォームなコマンドランナー**です。頻繁に使うコマンドを登録し、システムのどこからでも実行できます。
