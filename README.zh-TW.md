...
- **啟動速度快約17倍** 相較於基於Node.js的工作執行器
- **6.5ms啟動時間（平均）** 對比npm/yarn的115ms+大幅提升
- **4.5MB記憶體佔用** 對比Node.js的200MB+極為輕量

### 🔒 安全性

- **零Rust `eval()`** - 應用程式碼中無動態程式碼生成
- **安全的變數展開** - 無shell注入漏洞
- **細粒度控制** - 命令鏈接和子shell的分層權限系統
  - `allow_command_chaining` - 控制 `&&`、`||`、`;` 操作符（預設：停用）
  - `allow_subshells` - 控制正規表示式模式的 `()` 括號（預設：停用）
- **跳脫序列** - 支援 `\n`、`\r`、`\t`（安全風險最小）
- **相依性稽核** - 內建安全掃描
...
**獨特功能組合:**
- 🔒 零Rust eval()與fuzzing（373,423測試，0崩潰）
- 🌍 4語言支援（英/日/簡體中/繁體中）
...
cmdrun 是一個**快速、安全、跨平台的命令執行器**，讓您可以註冊並從系統的任何位置執行常用命令。
...
cmdrun 為 Zsh、Bash 和 Fish Shell 提供智慧補全功能，支援命令描述顯示和全域配置回退。
...
# Fish - 加入到 ~/.config/fish/config.fish
cmdrun completion fish | source
```

**功能：**
- 🎯 **智慧補全**：從全域和本機配置自動補全命令名稱
- 📝 **描述顯示**：顯示命令描述（Zsh/Fish）或命令清單（Bash）
- 🌍 **全域回退**：即使沒有本機 `commands.toml` 也能從全域配置工作
- ⚡ **快速**：最小開銷，速度最佳化
...
**語言配置：**
- 使用配置檔案設定（`commands.toml`）
- 變更語言：`cmdrun config set language <語言名>`
- 支援值：`english`、`japanese`、`chinese_simplified`、`chinese_traditional`
...
```toml
[config]
language = "chinese_traditional"  # 或 "english"、"japanese"、"chinese_simplified"
```
