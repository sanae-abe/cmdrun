...
- **启动速度快约17倍** 相比基于Node.js的任务运行器
- **6.5ms启动时间（平均）** 对比npm/yarn的115ms+大幅提升
- **4.5MB内存占用** 对比Node.js的200MB+极为轻量

### 🔒 安全性

- **零Rust `eval()`** - 应用程序代码中无动态代码生成
- **安全的变量展开** - 无shell注入漏洞
- **细粒度控制** - 命令链接和子shell的分层权限系统
  - `allow_command_chaining` - 控制 `&&`、`||`、`;` 操作符（默认：禁用）
  - `allow_subshells` - 控制正则表达式模式的 `()` 括号（默认：禁用）
- **转义序列** - 支持 `\n`、`\r`、`\t`（安全风险最小）
- **依赖审计** - 内置安全扫描
...
**独特功能组合:**
- 🔒 零Rust eval()与fuzzing（373,423测试，0崩溃）
- 🌍 4语言支持（英/日/简体中/繁体中）
