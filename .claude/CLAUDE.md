...
**pre_commit**:
- `cargo fmt` - 自動フォーマット（1-2秒）

**pre_push**:
- `cargo clippy --all-features -- -D warnings` - 静的解析優先（型エラー早期検出）
- `cargo test --all-features --lib --bins` - 単体テスト（統合テスト除外で10-30秒）

**設計根拠**:
- Clippy優先: 型エラーを数秒で検出（Fail Fast）
